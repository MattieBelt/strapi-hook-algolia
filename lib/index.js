    ...
    async initialize() {
      ...
      strapi.services.algolia = {
        ...
        saveObject: function (model, indexName) {
          ...
          initIndex(indexName).partialUpdateObject(model, {createIfNotExists: true})
            .then(() => {
              if (settings.debug) strapi.log.debug(`Saved object: ${model.objectID} on the Algolia Index: ${indexName}`);
            })
            .catch(error => {
